import { briscolaPoints, briscolaRankIndex, compareBriscola } from './cards.js';
import { tressettePoints, tressetteRankIndex, compareTressette } from './cards.js';

/**
 * AI Briscola: nessuna obbligazione di seme.
 * - Se secondo di mano: prova a prendere se conviene, altrimenti scarta basso
 * - Se primo: apre con scartina / carta di basso valore
 */
export function chooseBriscolaMove(hand, trick, trump, isFirst) {
  ...
  if (!isFirst && trick.length === 1) {
    ...
    // Prendi se la presa vale punti o se puoi prendere a costo basso
    ...
    // Scarta la carta di minor valore (preferendo non-briscole)
  ...
  // Primo di mano: evita di buttare carichi e briscole forti
  return hand.reduce((best, c) => {
    const cTrump = c.suit === trump;
    const bTrump = best.suit === trump;
    if (cTrump !== bTrump) return cTrump ? best : c;
